...
class AuthorForm(FlaskForm):
    ...
    biography = TextAreaField('Biography', validators=[
        DataRequired(), 
        Length(min=3, max=140, message="Biography must be between 3 and 140 chars")
        ])
    submit = SubmitField('Submit')
...
class GenreForm(FlaskForm):
    ...
    # - a submit button
    genre_name = StringField('Genre', validators=[
        DataRequired(),
        Length(min=3, max=80, message="Genre must be between 3 and 80 chars")
    ])
    submit = SubmitField('Submit')
